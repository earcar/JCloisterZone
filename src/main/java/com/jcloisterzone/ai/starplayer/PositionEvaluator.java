...
//  private Game game;
  ...
  private static final double[] openRoadPenalty = { 0.0, 0.2, 0.5, 2.5, 5.5, 9.5, 15.0, 22.0, 30.0 };
  private static final double[] openCityPenalty = { 0.0, 0.15, 0.3, 1.3, 2.3, 5.3, 10.0, 15.0, 22.0 };
  private static final double[] openFarmPenalty = { 0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0 };
  private static final double[] openCloisterPenalty = { 0.0, 0.0, 0.4, 0.8, 1.2, 2.0, 4.0, 7.0, 11.0 };
  private static final double[][] openPenalties = { openRoadPenalty, openCityPenalty, openFarmPenalty, openCloisterPenalty };
  ...
  private double rankFairy() {
    double rating = 0;
    if (! getGame().hasExpansion(Expansion.PRINCESS_AND_DRAGON)) rating = 0;
    return rating;
  }
...
//  }
  ...
  class PositionEvaluatorScoreAllCallback implements ScoreAllCallback {
    ...
    public CompletableScoreContext getCompletableScoreContext(Completable completable) {
      return new PositionEvaluatorScoreContext(getGame(), completable.getScoreContext(), scoreCache);
    }
    ...
    public void scoreCompletableFeature(CompletableScoreContext ctx) {
      rank += rankUnfishedCompletable(ctx.getMasterFeature(), (PositionEvaluatorScoreContext) ctx);
  ...
  protected double rankBuilder(Builder builder, PositionEvaluatorScoreContext ctx) {
  ...
  protected double rankUnfishedCompletable(Completable completable, PositionEvaluatorScoreContext ctx) {
    double rating = 0.0;
  ...
  protected double getUnfinishedCompletablePoints(Completable complatable, PositionEvaluatorScoreContext ctx) {
  ...
  protected double getUnfinishedCityPoints(City city, PositionEvaluatorScoreContext ctx) {
  ...
  protected double getUnfinishedRoadPoints(Road road, PositionEvaluatorScoreContext ctx) {
    double chanceToClose = ctx.getChanceToClose();;
  ...
  protected double getUnfinishedCloisterPoints(Cloister cloister, PositionEvaluatorScoreContext ctx) {
  ...
  protected double openObjectRating() {
    double rating = 0;
    int[] openCount = { openRoadCount, openCityCount, openFarmCount, openCloisterCount };
